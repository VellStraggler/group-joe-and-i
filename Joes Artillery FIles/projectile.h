...
public:
   ...
   void advance(double simulationTime)
   {
      ...
      next.pos.setMetersX(prev.pos.getMetersX() + next.v.getDX() * 1.0 + 0.5 * a.getDDX());
      next.pos.setMetersY(prev.pos.getMetersY() + next.v.getDY() * 1.0 + 0.5 * a.getDDY());

      // Step 4: Time
      next.t = simulationTime;

      // End sim if projectile out of range. 
      if (next.pos.getMetersY() < 0.0 || next.pos.getMetersY() > 5000.0 ||
         next.pos.getMetersX() < 0.0 || next.pos.getMetersX() > 50000.0)
      {
