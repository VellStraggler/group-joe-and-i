...
const double MIN_ALTITUDE = 100.0;  // min altitude is at 984'
const double MAX_ALTITUDE = 300.0; // max altitude is 3,000m or 9842.52ft
 ...
 ************************************************************************/
 void Ground :: reset(Position & posHowitzer)
 {
   ...
   ground[0] = posMinimum.getMetersY(); // the initial elevation is low
   ...
   for (int i = 1; i < width; i++)
   {
      ...
      { 
         ...
         if (ground[i] < 0.0 || ground[i] > posUpperRight.getPixelsY())
         {
            std::cerr << "ERROR: ground[" << i << "] = " << ground[i]
               << " (maxY = " << posUpperRight.getPixelsY() << ")\n";
         }
